...
% <single> <real/integer> <expensive/none>
...
% with Adaptation of Training Data Selection Criterion, in IEEE Symp. Ser. 
% Comput. Intell. (SSCI), Dec. 2022, pp. 1675–1682.
%------------------------------- Copyright --------------------------------
            ...
            while Algorithm.NotTerminated(Arc)
                ...
                if adapt_datacrtr
                    for j = 1:datacrtr_len
                        ...
                        % pred_Fs = Predict_SADEATDSC(test_Xs, surr, RBFN_lib, kernel);
                ...
                % Screen
                % cand_Fs = Predict_SADEATDSC(cand, surr, RBFN_lib, kernel);
                cand_Fs = surr(cand);
                [~, idx] = min(cand_Fs);
